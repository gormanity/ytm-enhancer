...
NATIVE_HOST_PATH="$INSTALL_ROOT/bin/ytme-native-host"
...
while IFS= read -r manifest_path; do
  if [[ -f "$manifest_path" ]] &&
    grep -Fq "\"path\": \"$NATIVE_HOST_PATH\"" "$manifest_path"; then
    rm -f "$manifest_path"
...
if [[ -L "$CLI_PATH" && "$(readlink "$CLI_PATH")" == "$INSTALL_ROOT/bin/ytme" ]]; then
  rm -f "$CLI_PATH"
