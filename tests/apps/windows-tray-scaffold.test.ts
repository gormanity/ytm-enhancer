...
describe("Windows tray connector scaffold", () => {
  ...
  it("uses a custom tray flyout instead of default dialog controls", () => {
    ...
    expect(popupForm).toContain("artistYearLabel.SetBounds(140, 80, 244, 20)");
    expect(popupForm).toContain("progressBar.SetBounds(24, 146, 376, 20)");
  ...
  it("captures Windows tray release screenshots from real connector playback", () => {
    ...
    expect(program).not.toContain('"ytm-tray-resource://demo-current-artwork"');
    expect(program).not.toContain('"ytm-tray-resource://demo-next-artwork"');
  ...
  it("packages prebuilt release zips without requiring the .NET SDK at install time", () => {
    ...
    expect(installScript).toContain('return "0.1.4"');
    ...
    expect(installScript).toContain(
      "Copy-ItemWithRetry -LiteralPath $PackagedExecutablePath",
    );
    expect(installScript).toContain(
      "Copy-ItemWithRetry -LiteralPath $PackagedReleaseMetadataPath",
    );
  ...
  it("generates a checksum update manifest for packaged Windows runtimes", async () => {
    ...
    const expectedVersion = process.env.YTM_WINDOWS_TRAY_VERSION ?? "0.1.4";
    const expectedBuildNumber = Number(
      process.env.YTM_WINDOWS_TRAY_BUILD_NUMBER ?? "1004",
    );
  ...
  it("reports the packaged assembly version through connector hello", () => {
    ...
    expect(protocol).not.toContain('ConnectorVersion = "0.1.4"');
    expect(coreProject).toContain("<Version>0.1.4</Version>");
  ...
  it("publishes component-scoped Windows tray releases", () => {
    ...
    const signingCheckWorkflow = readRepo(
    ...
    const codeSigningPolicy = readRepo("docs/code-signing-policy.md");
    ...
    expect(prepareCodesignScript).toContain("-Type CodeSigningCert");
    expect(prepareCodesignScript).toContain("CN=YTM Tray Beta Self-Signed");
    expect(prepareCodesignScript).toContain("Export-PfxCertificate");
    expect(prepareCodesignScript).toContain(
      "YTM_WINDOWS_TRAY_CODESIGN_CERTIFICATE_PASSWORD",
    );
    expect(prepareCodesignScript).toContain(
      "YTM_WINDOWS_TRAY_CODESIGN_CERTIFICATE_PATH",
    ...
    expect(workflow).not.toContain("WINDOWS_TRAY_CODESIGN_PFX_BASE64");
    expect(workflow).not.toContain("WINDOWS_TRAY_CODESIGN_PASSWORD");
    expect(signingCheckWorkflow).not.toContain(
      "WINDOWS_TRAY_CODESIGN_PFX_BASE64",
    );
    expect(signingCheckWorkflow).not.toContain(
      "WINDOWS_TRAY_CODESIGN_PASSWORD",
    );
