...
describe("PrecisionVolumeModule", () => {
  let module: PrecisionVolumeModule;
  ...
  beforeEach(() => {
    ...
    module = new PrecisionVolumeModule();
  });

  it("should have the correct module metadata", () => {
    expect(module.id).toBe("precision-volume");
    expect(module.name).toBe("Precision Volume");
  });
  ...
  it("should provide popup views", () => {
    ...
    expect(views[0].id).toBe("precision-volume-settings");
  });
