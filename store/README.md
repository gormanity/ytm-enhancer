...
- `dist/store-assets/description.txt`: paste-ready shared detailed description
...
3. Run `pnpm run build:store` to refresh the tracked PNGs and generate
   `dist/store-assets/`.
4. Paste `short-description.txt` and `description.txt` into each browser store.
   The detailed description is intentionally plain text for consistent results.
...
On macOS, copy either generated field directly to the clipboard:
