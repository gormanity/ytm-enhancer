...
$InstallUiDiagnosticsPath = Join-Path $WorkRoot "install-ui.json"
...
function Invoke-InteractiveInstallThroughUiAgent {
  ...
  $ScriptLines = @(
    ...
    "`$UiAgentClientPath = $(ConvertTo-PowerShellLiteral (Join-Path $PSScriptRoot "ui-agent-client.ps1"))",
    '. $UiAgentClientPath',
    "`$SetupPath = $(ConvertTo-PowerShellLiteral $SetupPath)",
    "`$ResultPath = $(ConvertTo-PowerShellLiteral $InstallResultPath)",
    "`$UiDiagnosticsPath = $(ConvertTo-PowerShellLiteral $InstallUiDiagnosticsPath)",
    ...
    "  }",
    '  $DialogElementCollection = $Dialog.FindAll(',
    '    [System.Windows.Automation.TreeScope]::Descendants,',
    ...
    '    for (',
    '      $ElementIndex = 0;',
    '      $ElementIndex -lt $DialogElementCollection.Count;',
    '      $ElementIndex++',
    "    ) {",
    ...
    "  )",
    '  if ($OkElements.Count -eq 0) {',
    '    throw "Installer completion dialog did not expose an OK control. Visible text: $DialogText"',
    "  }",
    '  $OkButtonWindowHandle = 0',
    '  foreach ($OkElement in $OkElements) {',
    '    $HandleCandidate = $OkElement',
    '    while ($null -ne $HandleCandidate -and $HandleCandidate -ne $Dialog) {',
    '      $CandidateHandle = $HandleCandidate.Current.NativeWindowHandle',
    "      if (",
    '        $CandidateHandle -ne 0 -and',
    "        (",
    '          $HandleCandidate.Current.ControlType -eq',
    '            [System.Windows.Automation.ControlType]::Button -or',
    '          $HandleCandidate.Current.ClassName -match "Button"',
    "        )",
    "      ) {",
    '        $OkButtonWindowHandle = $CandidateHandle',
    "        break",
    "      }",
    '      $HandleCandidate = `',
    '        [System.Windows.Automation.TreeWalker]::RawViewWalker.GetParent(',
    '          $HandleCandidate',
    "        )",
    "    }",
    '    if ($OkButtonWindowHandle -ne 0) {',
    "      break",
    "    }",
    "  }",
    ...
    "  }",
    '  Invoke-WindowsQaDialogOk `',
    '    -WindowHandle $Dialog.Current.NativeWindowHandle `',
    '    -ButtonHandle $OkButtonWindowHandle',
